    ...
    <view class="scroll-wrap" :style="{zIndex: zIndex}">
        ...
        >
            <view
                ...
                :key="index"
                :style="choseInd == item.id ? getActiveStyle : getDefaultStyle"
                class="scroll-view-item"
                :data-item="item.sp_name"
...
export default {
	...
	watch: {
		defaultChoseInd: function (newVal) {
			this.choseInd = newVal
		}
	},
    ...
    methods: {
        ...
        onClickScrollItem(index, event) {
            index === 0 ? this.choseInd = '' : this.choseInd = index
            const { item } = event.currentTarget.dataset
            const { offsetLeft } = event.target
            // console.log(this.windowWidth, 'this.windowWidth')
            this.position = offsetLeft - (this.windowWidth / 2 - 30)
            this.$emit('onValueChange', {currentInd: this.list[index].id, currentItem: item})
        },
        handleShowList() {
