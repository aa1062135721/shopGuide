		 ...
		 :style="{
			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap" @tap.stop.prevent="listClick(index)" :class="[uCurrent != index ? 'u-list-scale' : '']" :style="{
					...
					<image v-if="item[name] && item.type === 1" class="u-swiper-image" :src="item[name]" :mode="imgMode" style="height: auto; width: 610rpx;"></image>
					<video v-else-if="item[name] && item.type === 2" object-fit="contain" :id="`video${item.id}`"  :src="item[name]"  :loop="false" :enable-progress-gesture="false" :enable-play-gesture="true" style="height: 80%;"></video>
			...
			<view class="box">
				<view class="title flexBox">
					<view class="left flexBox">
						<text>{{cardData[current].title}}</text>
						<image src="@/static/icon_tuijian.png"></image>
					</view>
					<view class="right">
						<view class="single" v-for="(item1, index1) in cardData[current].actionArr" :key="index1">
							<text class="text">{{item1}}</text>
						</view>
						<!-- <view class="single">
							<text class="text">限时9折</text>
						</view><view class="single">
							<text class="text">全场半价</text>
						</view> -->
					</view>
				</view>
				<view class="address">
					<image src="@/static/icon_address.png"></image>
					<text>{{cardData[current].address}}</text>
				</view>					
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		updated () {
			...
			let videos = this.list.filter(v => v.type === 2)
